...
<div class="mb-6">
    ...
    <h2 class="mt-2 text-2xl font-bold text-gray-900">Editar Usuário</h2>
</div>
...
<div class="max-w-2xl rounded-lg bg-white p-6 shadow">
    <form method="POST" action="{{ route('admin.usuarios.update', $usuario->id) }}">
        ...
        <div class="mb-4">
            <label for="password" class="block text-sm font-semibold text-gray-700">Nova Senha (deixe em branco para manter)</label>
            <input
                type="password"
                id="password"
                name="password"
        ...
        <div class="mb-4">
            ...
            >
                ...
                @foreach(\App\Models\Restaurante::where('status', 'ativo')->get() as $restaurante)
                    <option value="{{ $restaurante->id }}" {{ old('restaurante_id', $usuario->restaurante_id) == $restaurante->id ? 'selected' : '' }}>
                        {{ $restaurante->nome }}
                    </option>
                @endforeach
            </select>
